...
<title>Log In - Continuity Inc. - Disaster Recovery Solutions</title>
<!-- InstanceEndEditable -->
...
//gets the place the user was at
if (isset($_GET['accesscheck']))
  $PrevUrl = $_GET['accesscheck'];
else
  $PrevUrl = $_POST['hfaccesscheck'];
  
//this will check to make sure that only after the user sube the form it will be use
if($_POST['hfSubmit'] != '')
{
	if(md5($_POST['txtImageStamp']) != $_SESSION['key'])
	{
		echo "<script type=\"text/javascript\">
			window.onload=function(){					
				displayMessage('divLogInPageError','Your CAPTCHA is incorrect.',true,true);
		}//end of window.onload=function()</script>";
	}//end of if
	else
	{
		if (isset($_POST['txtUserNamePage'])) 
		{
			if(logOnSite(base64_encode($_POST['txtUserNamePage']),base64_encode(base64_encode($_POST['txtPasswordNamePage'])),"",$database_conContinuty, $conContinuty) === FALSE)
			{
				echo "<script type=\"text/javascript\">
					window.onload=function(){
					  displayMessage('divLogInPageError','Your user name or password is incorrect.',true,true);
					  }
					</script>";
			}//end of if
			else
	...
	<div align="center">
        <div class="customContainer" id="divBasicContainer">
            <div class="customHeader" id="divBasicHeader">
              ...
              <h1><!-- InstanceBeginEditable name="h1Title" -->Log In<!-- InstanceEndEditable --></h1>
          	</div><!-- end of Header -->
            <div class="customContent" id="divBasicContent">
                <!-- InstanceBeginEditable name="BasicContent" -->
                <div class="customContainer" id="divMainBasicLogInContainer">
                	<div class="customContent" id="divMainBasicLogInContent">
                    	<div align="left" class="divPageTitle">
                			<label class="lblPageTitle">Log In</label>
                		</div>
                        <form action="<?php echo $LogInPage; ?>" method="post" name="frmLoginPage" id="frmLoginPage" class="frmBasics">
                            <div class="customContainer divBasicLogInContainer">
                                <div class="customHeader divBasicLogInHeader">
                                    <div id="divLogInPageError" class="divBasicMessage"></div>
                                </div>
                                <div class="customContent divBasicLogInContent">
                                    <label>User Name:</label>
                                </div>
                                ...
                                <div class="customFooter divBasicLogInFooter"></div>
                                <div class="customContent divBasicLogInContent">
                                    <input name="txtUserNamePage" type="text" id="txtUserNamePage" size="23" />
                                </div>
                                <div class="customNavigation divBasicLogInNavigation">
                                    <input name="txtPasswordNamePage" type="password" id="txtPasswordNamePage" size="20" />
                                </div>
                                <div class="customThridContent divBasicLogInThridContent">
                                    <input name="cmdSend" type="submit" value="Log In" id="cmdSend" />
                                </div>
                                <div class="customFooter divBasicLogInFooter"></div>
                            </div><!-- end of Cotinater -->
                            <div class="customContainer divBasicLogInContainer">
                                ...
                                <div class="customNavigation divBasicLogInNavigation">
                                    <input id="txtImageStamp" type="text" class="bginput" name="txtImageStamp" size="10"/>
                                </div>
                              <div class="customFooter divBasicLogInFooter">
                                  <img src="PurePHP/DynamicImage.php" alt="No CAPTCHA" name="imgReg" id="imgReg" />
                              </div>
                           </div>
                            
						   <input type="hidden" name="hfaccesscheck" value="<?php echo $PrevUrl; ?>" />
                           <input type="hidden" name="hfSubmit" value="1" />
                        </form>
                	</div>
                    <div class="customNavigation" id="divMainBasicLogInNavigation">
                    	<div align="left" class="divPageTitle">
                			<label class="lblPageTitle">Build A Plan Today</label>
                		</div>
                        <a href="SignUp.php?section=SignUp&Footer=1" onmouseout="changeImage('imgBuildYourPlan','images/build11side.jpg')" onmouseover="changeImage('imgBuildYourPlan','images/build12side.jpg')"><img src="images/build11side.jpg" id="imgBuildYourPlan" alt="Build A Plan Today" /></a>
                    </div>
                    <div class="customFooter" id="divMainBasicLogInFooter"></div>
                </div>
                <!-- InstanceEndEditable -->
            </div>
